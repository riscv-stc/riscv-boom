...
class SRT4DividerDataModule(len: Int) extends Module {
  ...
  val (a, b, sign, valid, kill_w, kill_r) =
    (io.src(0), io.src(1), io.sign, io.valid, io.kill_w, io.kill_r)
  val in_fire = valid && io.in_ready
  val out_fire = io.out_ready && io.out_valid

  val isHi = RegEnable(io.isHi, in_fire)
  val isW  = RegEnable(io.isW,  in_fire)
