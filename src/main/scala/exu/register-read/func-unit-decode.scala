...
object VecRRdDecode extends RRdDecodeConstants
{
  val table: Array[(BitPat, List[BitPat])] =
        ...
        ,BitPat(uopVFMV_V_F)   -> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_RS1,     OP2_ZERO,    IS_X, REN_1, CSR.N)
        ,BitPat(uopVMV_S_X)    -> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_RS1,     OP2_ZERO,    IS_X, REN_1, CSR.N)
        ,BitPat(uopVFMV_S_F)   -> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_RS1,     OP2_ZERO,    IS_X, REN_1, CSR.N)
        ,BitPat(uopVMV_X_S)    -> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_VS2,     OP2_ZERO,    IS_X, REN_1, CSR.N)
        ,BitPat(uopVFMV_F_S)   -> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_VS2,     OP2_ZERO,    IS_X, REN_1, CSR.N)
        ...
        ,BitPat(uopVSLIDE1DOWN)-> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_VS2 ,    OP2_ZERO,    IS_X, REN_1, CSR.N)
        ...
        ,BitPat(uopMMV_T)      -> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_VS1,     OP2_ZERO,    IS_X, REN_1, CSR.N)
        ,BitPat(uopMWMV_T)     -> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_VS1,     OP2_ZERO,    IS_X, REN_1, CSR.N)
        ,BitPat(uopMQMV_T)     -> List(BR_N, Y, N, N, FN_ADD,   DW_XPR, OP1_VS1,     OP2_ZERO,    IS_X, REN_1, CSR.N)
  )
}
...
object CsrRRdDecode extends RRdDecodeConstants
{
  val table: Array[(BitPat, List[BitPat])] =
         ...
         BitPat(uopMSETTYPEI)  -> List(BR_N , Y, N, N, FN_ADD , DW_XPR, OP1_ZERO, OP2_ZERO  , IS_I, REN_1, CSR.N),
...
object FpuRRdDecode extends RRdDecodeConstants
{
  val table: Array[(BitPat, List[BitPat])] =
         ...
         BitPat(uopFCLASS_H)->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
...
//         BitPat(uopFMV_D_X)->List(BR_N , Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         ...
         BitPat(uopFMV_X_H)->List(BR_N , Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         ...
         BitPat(uopFSGNJ_H)->List(BR_N , Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         ...
         BitPat(uopFCVT_H_S) ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFCVT_H_D) ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFCVT_S_H) ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFCVT_D_H) ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
...
// TODO comment out I2F instructions.
         ...
         BitPat(uopFCVT_H_W) ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFCVT_H_L) ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFCVT_W_H) ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFCVT_L_H) ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopCMPR_H)   ->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFMINMAX_H)->List(BR_N,Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFADD_H)  ->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFSUB_H)  ->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFMUL_H)  ->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFNMSUB_D)->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFMADD_H) ->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFMSUB_H) ->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFNMADD_H)->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
         BitPat(uopFNMSUB_H)->List(BR_N, Y, N, N, FN_X   , DW_X  , OP1_X   , OP2_X   , IS_X, REN_1, CSR.N),
...
object MatRRdDecode extends RRdDecodeConstants
{
  val table: Array[(BitPat, List[BitPat])] =
             Array[(BitPat, List[BitPat])](
                                    ...
                                    // |     |  |  |  alu fcn   wd/word?|         |          |     |      |
                                    // |     |  |  |  |         |       |         |          |     |      |
         BitPat(uopMMA)        -> List(BR_N, Y, N, N, FN_ADD,  DW_XPR,  OP1_TS1,  OP2_TS2,   IS_X, REN_1, CSR.N)
        ,BitPat(uopMWMA)       -> List(BR_N, Y, N, N, FN_ADD,  DW_XPR,  OP1_TS1,  OP2_TS2,   IS_X, REN_1, CSR.N)
        ,BitPat(uopMQMA)       -> List(BR_N, Y, N, N, FN_ADD,  DW_XPR,  OP1_TS1,  OP2_TS2,   IS_X, REN_1, CSR.N)
        ,BitPat(uopMFNCVT)     -> List(BR_N, Y, N, N, FN_ADD,  DW_XPR,  OP1_ACC,  OP2_ZERO,  IS_X, REN_1, CSR.N)
        ,BitPat(uopMMV_V)      -> List(BR_N, Y, N, N, FN_ADD,  DW_XPR,  OP1_ACC,  OP2_ZERO,  IS_X, REN_1, CSR.N)
        ,BitPat(uopMWMV_V)     -> List(BR_N, Y, N, N, FN_ADD,  DW_XPR,  OP1_ACC,  OP2_ZERO,  IS_X, REN_1, CSR.N)
        ,BitPat(uopMQMV_V)     -> List(BR_N, Y, N, N, FN_ADD,  DW_XPR,  OP1_ACC,  OP2_ZERO,  IS_X, REN_1, CSR.N)
      )
}
  ...
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  if (usingMatrix) {
    ...
    io.rrd_uop.ctrl.is_sta  := io.rrd_uop.uopc.isOneOf(uopSTA, uopVSA, uopVSMA, uopVSSA, uopVSUXA, uopVSOXA, uopAMO_AG, uopMSE, uopMSFD, uopMSE_V)
    ...
    io.rrd_uop.ctrl.is_unfold := io.rrd_uop.uopc.isOneOf(uopMLUF, uopMSFD)
    ...
    when(io.rrd_uop.uopc.isOneOf(uopMMV_V) && io.rrd_uop.rt(RS1, isTrTile)) {
      io.rrd_uop.ctrl.op1_sel := OP1_TS1
    }
  } else if (usingVector) {
    io.rrd_uop.ctrl.is_load := io.rrd_uop.uopc.isOneOf(uopLD, uopVL, uopVLM, uopVLFF, uopVLS, uopVLUX, uopVLOX)
    io.rrd_uop.ctrl.is_sta  := io.rrd_uop.uopc.isOneOf(uopSTA, uopVSA, uopVSMA, uopVSSA, uopVSUXA, uopVSOXA, uopAMO_AG)
    io.rrd_uop.ctrl.is_std  := io.rrd_uop.uopc === uopSTD || (io.rrd_uop.ctrl.is_sta && io.rrd_uop.rt(RS2, isInt) && !io.rrd_uop.is_rvv)
    io.rrd_uop.ctrl.is_vmlogic  := io.rrd_uop.uopc.isOneOf(uopVMAND, uopVMNAND, uopVMANDNOT, uopVMXOR, uopVMOR, uopVMNOR, uopVMORNOT, uopVMXNOR)
    io.rrd_uop.ctrl.is_vmscmp   := io.rrd_uop.uopc.isOneOf(uopVMSEQ, uopVMSNE, uopVMSLTU, uopVMSLT, uopVMSLEU, uopVMSLE, uopVMSGTU, uopVMSGT)
    when(io.rrd_uop.uopc.isOneOf(uopVDIV, uopVDIVU, uopVREM, uopVREMU)) {
