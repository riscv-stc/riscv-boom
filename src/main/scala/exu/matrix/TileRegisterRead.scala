...
)(implicit p: Parameters) extends BoomModule with freechips.rocketchip.tile.HasFPUParameters
{
  ...
  for (w <- 0 until issueWidth) {
    ...
    io.tileReadPorts(idx+0).tilewidth := io.iss_uops(w).mconfig.tile_m
    io.tileReadPorts(idx+0).tt        := 3.U                  // tr_c
    ...
    io.tileReadPorts(idx+1).tilewidth := io.iss_uops(w).mconfig.tile_m
    io.tileReadPorts(idx+1).tt        := 2.U                  // tr_r
